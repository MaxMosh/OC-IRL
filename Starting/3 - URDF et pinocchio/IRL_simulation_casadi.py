...
from tools import steps_two_arms_robot_casadi


results_angle_casadi, results_vitesses_angulaires_casadi = steps_two_arms_robot_casadi.solve_DOC(w = steps_two_arms_robot_casadi.w_true)
...
print(dq_op.shape)
dPx_op = np.zeros((1,steps_two_arms_robot_casadi.N)).T
print(dPx_op.shape)

# for i in range(steps_two_arms_robot_casadi.N):
#     dPx_op[i,:] = steps_two_arms_robot_casadi.dPx_fun(q_op[i,:], dq_op[i,:])
...
J_opt = steps_two_arms_robot_casadi.cost_function_value(q_op.T, dq_op.T)    #, dPx_op.T)
J_nopt = np.zeros((5, nb_nopt))
# J_nopt = []
for i in range(nb_nopt):
    J_nopt[:,i] = steps_two_arms_robot_casadi.cost_function_value(q_nopts[:,:,i], dq_nopts[:,:,i])      #, dPx_nopts[:, :, i])
candidates_costs_true = steps_two_arms_robot_casadi.w_true.T @ J_nopt
candidates_and_expert_costs_true = np.append(candidates_costs_true, steps_two_arms_robot_casadi.w_true.T @ J_opt)
...
for i in range(nb_iter_IRL):
    ...
    w_est_current = steps_two_arms_robot_casadi.IRL_step(J_expert_trajectories_current, J_nopt_augmented)
    print(w_est_current)

    # Calcul de la trajectoire optimale avec w_est
    results_angles_casadi_current, results_vitesses_angulaires_casadi_current = steps_two_arms_robot_casadi.solve_DOC(w = np.concatenate((w_est_current, np.array([0.0, 0.0]))))

    # Calcul du vecteur \Phi
    cost_features_new_traj = steps_two_arms_robot_casadi.cost_function_value(results_angles_casadi_current.T, results_vitesses_angulaires_casadi_current.T) #, dPx_nopts.T)
...
Px_opt = steps_two_arms_robot_casadi.L_1*ca.cos(q_op[-1,0]) + steps_two_arms_robot_casadi.L_2*ca.cos(q_op[-1,0] + q_op[-1,1])
Px_last_IRL = steps_two_arms_robot_casadi.L_1*ca.cos(results_angles_list[0,-1,-1]) + steps_two_arms_robot_casadi.L_2*ca.cos(results_angles_list[0,-1,-1] + results_angles_list[1,-1,-1])
