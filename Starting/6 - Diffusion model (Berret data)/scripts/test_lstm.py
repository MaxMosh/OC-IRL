...
CONTEXT_LENGTH = 10  # Number of timesteps to use as context for prediction                     # INITIALLY 20
PREDICTION_LENGTH = 40  # Number of timesteps to predict                                        # INITIALLY 10
...
# ==================== LOAD TEST DATA ====================
def load_test_trajectories(test_dir="data/S18"):                                        # INITIALLY S21
    """
    Load all trajectory files from test directory (S18).                                # INITIALLY S21
...
def plot_test_predictions(model, test_paths, context_length, prediction_length, 
    ...
    plt.suptitle('Test Set Predictions (S18 - Unseen Data)', fontsize=14, fontweight='bold')        # INITIALLY S21
    plt.tight_layout()
...
def plot_long_term_predictions(model, test_paths, context_length, num_predictions=3, 
    ...
    for path in test_paths:
        ...
        if len(traj) >= context_length + prediction_steps:
            trajectories.append(traj)
    
    if len(trajectories) == 0:
        print("No suitable trajectories for long-term prediction!")
        return None
    ...
    for idx, traj_idx in enumerate(selected_indices):
        ...
        ground_truth = trajectory[:context_length + prediction_steps]
...
def main():
    ...
    """
    print("="*70)
    print("LSTM TRAJECTORY PREDICTION - TEST ON S18")                                           # INITIALLY S21 
    print("="*70)
    print(f"Device: {DEVICE}")
    
    # ========== LOAD MODEL ==========
    model_path = "trained_models/lstm_trajectory_20251113_183212_lstm_epochs_10_cont_10_pred_40_added_smoothness.pth"  # UPDATE THIS!
    ...
    # ========== LOAD TEST DATA ==========
    print("\n" + "="*70)
    print("Loading test data from S18...")                                              # INITIALLY S21
    print("="*70)
    
    try:
        test_paths = load_test_trajectories("data/S18")                                 # INITIALLY S21
    except FileNotFoundError as e:
        print(f"\nERROR: {e}")
        print("Make sure the data/S18 directory exists with test trajectories.")        # INITIALLY S21
        return
    ...
    print(f"\nTest Set Results (S18 - Unseen Data):")                                   # INITIALLY S21
    ...
    mse_fig = plot_mse_per_step(results['mse_per_step'], 
                                title="MSE per Prediction Step (Test Set - S18)")               # INITIALLY S21
