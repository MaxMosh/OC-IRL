...
# Load Model
# Initialize with input_channels=4 (The TransformerEncoder will handle this dim)
model = ConditionalDiffusionModel(w_dim=W_DIM, input_channels=INPUT_CHANNELS).to(DEVICE)

# Load the trained weights
# On pointe vers le dossier checkpoints défini dans le train.py
model_path = "checkpoints/diffusion_model_final.pth" 
# Ou un epoch spécifique si besoin :
...
# NOTE: Even with the Transformer, we keep this shape.
# The TransformerTrajEncoder.forward() method expects (Batch, 4, 50) 
# and will internally permute it to (Batch, 50, 4).
