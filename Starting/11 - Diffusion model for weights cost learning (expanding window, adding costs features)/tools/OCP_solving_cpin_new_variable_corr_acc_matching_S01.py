...
# Lengths
# L_1 = 1
L_1 = 3.187993150492783312e-01
# L_2 = 1
L_2 = 4.237188625476386861e-01
N_angles = 2
...
# NEW: Acceleration limits (arbitrary large values if unknown, adjust as needed)
...
# RNEA now uses the real 'acc_sym' instead of zeros implicitly in the solver
...
def cost_function_value(q_var, dq_var, ddq_var, N_steps):
    ...
    Added ddq_var to compute dynamic torque.
    """
    f_10 = []
    f_11 = []
    f_2 = []
    ...
    for t in range(N_steps):
        ...
        acc_t = ca.vertcat(ddq_var[0, t], ddq_var[1, t]) # NEW: Get acceleration
        
        f_10.append(dq_var[0, t]**2)
        f_11.append(dq_var[1, t]**2)
        f_2.append(vx_fun(q_t, dq_t)**2)
        
        # Calculate Tau with REAL acceleration
...
def solve_DOC(w_matrix, N_steps, x_fin=0.95*(L_1 + L_2), q_init=[-np.pi/2, np.pi/2], verbose=False):
    """
    Solves the OCP WITH Acceleration as a variable.
    """
    opti = ca.Opti()
    ...
    ddq = opti.variable(N_angles, N_steps) # NEW: Acceleration variable

    # Pass ddq to the cost function
    ...
    for t in range(N_steps - 1):
        ...
        # Velocity integration (NEW: depends on acceleration)
        ...
        # Acceleration Limits (NEW)
        # Uncomment if you want strict limits, otherwise let the cost function handle it
        # opti.subject_to(opti.bounded(ddq_min_lim_rad_par_s2, ddq[0, t], ddq_max_lim_rad_par_s2))
        # opti.subject_to(opti.bounded(ddq_min_lim_rad_par_s2, ddq[1, t], ddq_max_lim_rad_par_s2))

    # Boundary Conditions
    opti.subject_to(q[:, 0] == q_init)
    # Start from 0 velocity ? (Optional, uncomment if needed)
...
def plot_joint_dynamics(joint_id, list_q, list_dq, list_ddq=None, linestyle='-', save_path=None):
    """
    Trace q, dq et ddq pour un joint donné et sauvegarde la figure si save_path est fourni.
    """
    # Configuration des sous-graphiques (3 lignes si accélération, sinon 2)
    rows = 3 if list_ddq is not None else 2
    fig, axs = plt.subplots(rows, 1, figsize=(10, 8), sharex=True)
    
    fig.suptitle(f"Dynamique Joint {joint_id + 1}")
    ...
    # Si le backend n'est pas "Agg", on affiche aussi
    if matplotlib.get_backend() != 'Agg':
        plt.show()
    plt.close(fig) # Ferme la figure pour libérer la mémoire


def plot_trajectory_q1(list_q, list_dq, list_ddq=None, linestyle='-', save_path='data/trajectory_q1_matching_S01.png'):
    plot_joint_dynamics(0, list_q, list_dq, list_ddq, linestyle, save_path)


def plot_trajectory_q2(list_q, list_dq, list_ddq=None, linestyle='-', save_path='data/trajectory_q2_matching_S01.png'):
    plot_joint_dynamics(1, list_q, list_dq, list_ddq, linestyle, save_path)


def plot_trajectory_ee(list_q, x_fin_target=0.95*(L_1 + L_2), linestyle='-', save_path='data/trajectory_ee_matching_S01.png'):
