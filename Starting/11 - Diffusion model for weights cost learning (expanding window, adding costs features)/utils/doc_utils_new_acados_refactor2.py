...
class DocHumanMotionGeneration_InvDyn:
    ...
    # # # terminal cost (recopy of path cost but we keep it like that for clarity)
        ...
        if param.free_flyer:
            ...
            # Add initial configuration as parameter (Option B)
            p_qdi = casadi.SX.sym('p_qdi', nq)
            self.idx_p_qdi = 1  # starts right after selector
            ocp.model.p = casadi.vertcat(ocp.model.p, p_qdi)

            # Append BOS(6) so they are the LAST 6 in p
            p_bos = casadi.SX.sym('p_bos', 6)
            ocp.model.p = casadi.vertcat(ocp.model.p, p_bos)

            # parameter_values of correct length (1 + nq + 6)
            ocp.parameter_values = np.zeros(int(ocp.model.p.size1()))

            # Enforce q(0) = p_qdi (initial joint configuration as runtime parameter)
...
# ---------------------------
            ...
            # Non-free-flyer: use qdi as parameter (Option B)
            p_qdi = casadi.SX.sym('p_qdi', nq)
            self.idx_p_qdi = 0
            ocp.model.p = p_qdi
            ocp.parameter_values = np.zeros(int(ocp.model.p.size1()))
