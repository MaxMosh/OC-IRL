...
# MODEL: RNN-based diffusion with conditioning on first 20 frames
...
class ConditionalTrajectoryDataset(torch.utils.data.Dataset):
    def __init__(self, data_paths, context_length=CONTEXT_LENGTH, 
        ...
        """
        Load trajectory data for conditional generation.
        Each trajectory is split into context (first 20 frames) and prediction (next 80 frames).
        
        Args:
            data_paths: list of paths to CSV files
            context_length: number of frames to use as context
            prediction_length: number of frames to predict
        ...
        for path in tqdm(data_paths, desc="Loading trajectories"):
            try:
                ...
                if len(trajectory) >= self.total_length:
                    # Extract context and prediction parts
...
def main():
    ...
    train_dataset = ConditionalTrajectoryDataset(train_paths)
    val_dataset = ConditionalTrajectoryDataset(val_paths)
