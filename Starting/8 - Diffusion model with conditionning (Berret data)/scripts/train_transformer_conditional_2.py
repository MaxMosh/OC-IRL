...
class ConditionalTransformerBlock(nn.Module):
    ...
    def forward(self, x, t_emb, c_emb):
        ...
        Args:
            ...
            c_emb: (batch, d_model) - context embedding
        """
        # Add time and context embeddings to input
        t_emb = self.time_mlp(t_emb).unsqueeze(1)  # (batch, 1, d_model)
        c_emb = self.context_mlp(c_emb).unsqueeze(1)  # (batch, 1, d_model)
        x = x + t_emb + c_emb
...
class ConditionalDiffusionTransformer(nn.Module):
    ...
    def __init__(self, trajectory_dim=TRAJECTORY_DIM, 
        ...
        # Transformer blocks with context conditioning
    ...
    def forward(self, x_noisy, context, t):
        ...
        # Encode context
        context_h = self.context_input_proj(context)  # (batch, context_length, d_model)
        context_h = self.context_pos_encoding(context_h)
        context_h = self.context_encoder(context_h)  # (batch, context_length, d_model)
        context_flat = context_h.view(batch_size, -1)  # (batch, context_length * d_model)
        context_emb = self.context_pooling(context_flat)  # (batch, d_model)
        
        # Create time embedding
        t_emb = self.timestep_embedding(t.float(), self.time_dim)  # (batch, time_dim)
        t_emb = self.time_mlp(t_emb)  # (batch, d_model)
        
        # Project noisy prediction to d_model dimension
        h = self.input_proj(x_noisy)  # (batch, prediction_length, d_model)
        
        # Add positional encoding
        h = self.pos_encoding(h)
        
        # Apply transformer blocks with context conditioning
...
def main():
    ...
    # Plot losses
    plot_losses(train_losses, val_losses, save_path='conditional_training_losses_transformer.png')
    
    # Save model
    print("\nSaving Model...")
    os.makedirs("trained_models", exist_ok=True)
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    model_path = f"trained_models/conditional_diffusion_transformer_{timestamp}.pth"
    torch.save({
    ...
    fig.savefig(f'conditional_generation_transformer_{timestamp}.png', dpi=150, bbox_inches='tight')
    plt.show()
