...
def solve_DOC(w, x_fin = -1.0, q_init = [0, ca.pi/4]):
    ...
    J_10, J_11, J_2 = cost_function_value(q, dq)
    ...
    for t in range(N):
        ...
        J_tot += w[t,0] * J_10[t] + w[t,1] * J_11[t] + w[t,2] * J_2[t]
        # if len(w) >= 5:
...
def generate_DOC_solutions_list_w(array_w, x_fin, q_init, remove_bad_w = True):
    ...
    print(f"Number of weights sequence: {nb_w}")
    number_of_added = 0
    number_of_not_added = 0
    results_angles_list = []
    results_vitesses_angulaires_list = []
    ...
    for ind_w, w in enumerate(array_w):
        ...
        results_vitesses_angulaires_list.append(results_vitesses_angulaires_cpin)
        number_of_added+=1
        print(f"Current shape of valid list of w: {good_w.shape}")
        print(f"Current shape of w: {w.T.shape}")
