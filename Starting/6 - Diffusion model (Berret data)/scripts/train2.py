...
class MLPBlock(nn.Module):
    """
    MLP block with residual connection and time embedding.
    """
...
class DiffusionMLP(nn.Module):
    """
    MLP-based diffusion model for trajectory generation.
    """
    def __init__(self, trajectory_dim=TRAJECTORY_DIM, max_length=MAX_LENGTH, 
        ...
        # Flatten input: (batch, max_length, trajectory_dim) -> (batch, max_length * trajectory_dim)
        self.input_dim = max_length * trajectory_dim
        ...
        self.blocks = nn.ModuleList([
            MLPBlock(hidden_dim, time_dim) for _ in range(num_layers)
        ])
    ...
    def forward(self, x, t):
        """
        Args:
            x: (batch, max_length, trajectory_dim) - noisy trajectory
            t: (batch,) - timestep
            
        Returns:
            (batch, max_length, trajectory_dim) - predicted noise
        """
        batch_size = x.shape[0]
        
        # Flatten trajectory
        x_flat = x.reshape(batch_size, -1)  # (batch, max_length * trajectory_dim)
        ...
        output = self.output_proj(h)  # (batch, max_length * trajectory_dim)
        
        # Reshape to trajectory format
        output = output.reshape(batch_size, self.max_length, self.trajectory_dim)
