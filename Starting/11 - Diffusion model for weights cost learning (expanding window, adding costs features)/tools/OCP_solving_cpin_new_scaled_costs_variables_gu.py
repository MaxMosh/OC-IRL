...
mean_q_init_1 = [-90,90]
mean_q_init_2 = [-15,105]
mean_q_init_3 = [-115,115]
mean_q_init_set_deg = np.array([mean_q_init_1, mean_q_init_2, mean_q_init_3])
mean_q_init_set_rad = np.radians(mean_q_init_set_deg)

uncertainty_deg = 20
uncertainty_rad = np.radians(uncertainty_deg)
...
def compute_scaling_factors(num_samples=10, x_fin_def=1.9, q_init_def=[-np.pi/2, np.pi/2]):
    """
    ...
    for i in range(num_samples):
        ...
        mean_q_init = np.random.choice(mean_q_init_set_rad)
        mean_q_init = np.array(mean_q_init)
        q_init = mean_q_init + np.random.normal(0,(1/30)*uncertainty_rad, mean_q_init.shape)
        # MOST OF THE ANGLES WILL BE WITHIN +/- 21°

        x_fin = x_fin_def + np.random.normal(0,0.01)
        try:
...
def solve_DOC(w, x_fin=1.9, q_init=[-np.pi/2, np.pi/2], scale_factors=None, verbose=False):
    ...
    t_0 = 0
    nb_samples = np.randint(80,200)
    t_f = (1/100)*nb_samples
    ...
    q = opti.variable(N_angles, nb_samples)
    dq = opti.variable(N_angles, nb_samples)
    ...
    for t in range(nb_samples):
        ...
        term_10 = w[0,t] * (J_10[t] * scale_factors["J_10"])
        term_11 = w[1,t] * (J_11[t] * scale_factors["J_11"])
        term_2  = w[2,t] * (J_2[t]  * scale_factors["J_2"])
        
        J_tot += term_10 + term_11 + term_2

        if len(w) >= 5:
            term_tau    = w[3,t] * (J_tau[t]    * scale_factors["J_tau"])
            term_energy = w[4,t] * (J_energy[t] * scale_factors["J_energy"])
...
def generate_DOC_solutions_list_w(array_w, x_fin, q_init, scale_factors=None, remove_bad_w=True):
    ...
    good_w = np.empty((0, array_w.shape[1], array_w.shape[2]))
