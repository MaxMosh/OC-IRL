...
f_expl=dyn_fun(cx,cu)#casadi.vertcat(cx[model.nq:],acc(cx,cu)) # composed of dq and ddq(from aba)
...
# # ocp.solver_options.regularize_method = 'MIRROR' # if SQP tehn regularize the hessian
...
p_mat= 1e2*np.eye(3) #1e3*np.eye(3) 
# Path cost external scalar expression
cost_vec = casadi.vertcat(cx[:model.nq], cx[model.nq:], cu, cost_to_target(cx, cu)[0:3])
W = casadi.diagcat(q_mat, dq_mat, tau_mat, p_mat)
ocp.cost.cost_type = 'EXTERNAL'
ocp.model.cost_expr_ext_cost  =  0.5*casadi.mtimes([cost_vec.T, W, cost_vec])
ocp.model.cost_expr_ext_cost_0 = ocp.model.cost_expr_ext_cost  # for stage 0

# # Terminal cost external scalar expression
cost_vec_e = cost_to_target(cx, cu)[0:3]
W_e = 10 * p_mat
ocp.cost.cost_type_e = 'EXTERNAL'
ocp.model.cost_expr_ext_cost_e= 0.5*casadi.mtimes([cost_vec_e.T, W_e, cost_vec_e])
...
ocp.model.nh_e = 3
ocp.model.con_h_expr_e = casadi.vertcat(cost_to_target(cx)[0:3])
