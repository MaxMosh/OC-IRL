...
for i1 in range(N_grid_w):
    if ((i1 > N_grid_w/20) and (i1 < N_grid_w - 1)):
        ...
        for i2 in range(temp_range_i2):
            if i2 > N_grid_w/20:
                i3 = N_grid_w - 1 - i1 - i2
                w_current = [(1/(N_grid_w-1))*i1, (1/(N_grid_w-1))*i2, (1/(N_grid_w-1))*i3]
...
if (array_w.shape[0] == array_results_angles.shape[0]):
    np.save(f'data/array_w_{N_grid_w}.npy', array_w)
    np.save(f'data/array_results_angles_{N_grid_w}.npy', array_results_angles)