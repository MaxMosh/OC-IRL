...
class TrajectoryEncoder(nn.Module):
    ...
    Uses 1D Convolutions since data is sequential/temporal (because the data is a trajectory,
    strong correlations between consecutive points).
    """
    def __init__(self, input_channels=5, sequence_length=50, embedding_dim=64):
        super().__init__()
...
class SinusoidalPositionEmbeddings(nn.Module):
    ...
    TODO: have to check if it's relevant
    """
...
class DenoisingNetwork(nn.Module):
    ...
    NOTE: MLP seems to be the best option for finding the weights,
    as they are note especially correlated to each other (unlike for the motion forecasting)
    """
    def __init__(self, w_dim=5, cond_dim=64, time_emb_dim=32):
        ...
        self.net = nn.Sequential(
            ...
            nn.Linear(256, w_dim) # Output matches w dimension (it could be 3 or 5)
        )
