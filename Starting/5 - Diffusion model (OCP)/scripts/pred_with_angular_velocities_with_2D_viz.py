...
model_path = "Starting/5 - Diffusion model/trained_models/trained_diffusion_model_with_velocities_n_x_fin_101_2025-10-29_16:22.pt"

stats = np.load("Starting/5 - Diffusion model/data/stats_with_velocities_n_linspace_101.npy", allow_pickle=True).item()
...
n_samples = 90 #TODO: ADDED, NEED TO SEE IF IT WORKS WITH ELSE THAN 50                      # MODIFIED: INITIALLY 50
@torch.no_grad()
...
data = np.load("Starting/5 - Diffusion model/data/trajectories_dataset_with_velocities_n_linspace_101.npy", 
               allow_pickle=True)
example_traj = data[0]["future"]  # shape (N, 4)
# example_traj = data[0]["prefix"]
...
samples = sample_with_prefix(model, prefix, n_total=n_total, n_samples=n_samples)
...
for i, (ax, title, color) in enumerate(zip(axs, titles, colors)):
    ...
    # DOC_traj = np.concatenate((data[0]["prefix"], data[0]["future"]))
    # print(DOC_traj.shape)
    # ax.plot(time, DOC_traj[:, i], "--", 
    #         color="black", alpha=0.6, linewidth=1.5, label="DOC trajectory")
