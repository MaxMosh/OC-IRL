...
class DocHumanMotionGeneration_InvDyn:
    ...
    def __init__(self,model,param):
        ...
        # Target orientations are constant; target positions will be passed as ACADOS parameters (p)
        self.FOI_R = [casadi.DM(param.FOI_orientation[i]) for i in range(len(param.FOI_orientation))]
        self.n_foi = len(self.FOI_R)
        self.FOI_ids = list(param.FOI_to_set_Id)
        ...
        # Target position errors are built on-the-fly in `cost_to_target_expr(cx, cu, p)`.
    ...
    def solve_doc_acados(self,param):
        ...
        q0=param.qdi# NEED TO BE PASSED AS A PARAMETERS !
        
        if param.free_flyer:
            F_lf, F_rf = self._compute_initial_contact_wrenches(q0)
        else:
            F_lf, F_rf = None, None
       ...
       # -------------------------------------
        ...
        # ACADOS parameters (p)
        # Layout:
        #   free_flyer: [s_tgt0(1), qdi(nq), foi_pos(3*n_foi), bos(6)]
        #   fixed base: [qdi(nq), foi_pos(3*n_foi)]
        # -----------------------
        n_foi = len(param.FOI_orientation)
        if param.free_flyer:
            s_tgt0 = casadi.SX.sym('s_tgt0', 1)
            p_qdi  = casadi.SX.sym('p_qdi', nq)
            p_foi  = casadi.SX.sym('p_foi', 3*n_foi)
            p_bos  = casadi.SX.sym('p_bos', 6)
            self.idx_s_tgt0 = 0
            self.idx_p_qdi  = 1
            self.idx_p_foi  = 1 + nq
            ocp.model.p = casadi.vertcat(s_tgt0, p_qdi, p_foi, p_bos)
        else:
            p_qdi  = casadi.SX.sym('p_qdi', nq)
            p_foi  = casadi.SX.sym('p_foi', 3*n_foi)
            self.idx_p_qdi = 0
            self.idx_p_foi = nq
            ocp.model.p = casadi.vertcat(p_qdi, p_foi)
        ...
        cost_terms, W_blocks, slice_map, target_slices = self.calculate_cost(cx, cu, param, ocp.model.p) # go to declaration to add more cost functions
    ...
    # # # terminal cost (recopy of path cost but we keep it like that for clarity)
        ...
        for i in range(len(param.FOI_to_set_Id) ):
            block_i = self.cost_to_target_expr(cx, cu[:nv], ocp.model.p)[i*6:(i+1)*6]
        ...
        if param.free_flyer:
            ...
            tol = 5e-3
            # Selector is p[0] (see p-layout above)
            s_tgt0 = ocp.model.p[self.idx_s_tgt0]

            # Constraint: target0 z only (scaled by tol, gated by selector)
            z_err_t0 = self.cost_to_target_expr(cx, cu[:self.nv], ocp.model.p)[2]
            expr_tgt0 = casadi.vertcat(s_tgt0 * (z_err_t0 / tol))
            ...
            # Non-free-flyer: q(0) is constrained to the parameter p_qdi (see p-layout above)

            # Enforce q(0) = p_qdi

            ocp.model.con_h_expr_0 = cx[:nq] - ocp.model.p[self.idx_p_qdi:self.idx_p_qdi + nq]
            ocp.constraints.lh_0 = np.zeros(nq)
            ocp.constraints.uh_0 = np.zeros(nq)
...
# Bos parameters always at the end of parameters vector
        if param.free_flyer:
         ...
         # Convert to np arrays
            ...
            # qdi parameter block (after selector)
            p_base[self.idx_p_qdi:self.idx_p_qdi + nq] = np.asarray(param.qdi, dtype=float).reshape(nq,)
            # FOI positions (flattened)
            p_base[self.idx_p_foi:self.idx_p_foi + 3*n_foi] = np.asarray(param.FOI_position, dtype=float).reshape(3*n_foi,)
        ...
        # initial constraints on the state vector at node 0
        ocp.constraints.idxbx_0 = np.arange(0, nq+nv)
        ocp.constraints.lbx_0   = np.zeros(nq+nv)
        ocp.constraints.ubx_0   = np.zeros(nq+nv)
     ...
     # set options
        ...
        x_init0 = np.concatenate([param.qdi, np.zeros(self.nv)])
        ocp_solver.set(0, "x", x_init0)

        # optionally also set x0 guess for acados (harmless, helps clarity)
        ocp.constraints.x0 = x_init0
        
        x0 = np.concatenate([param.qdi, np.zeros(nv)])
        ...
        if param.free_flyer:
            ...
            for k in range(param.nb_samples + 1):   # include terminal stage
                ...
                # selector
                p_vec[self.idx_s_tgt0] = 1.0 if k == k_star else 0.0
                # qdi and FOI positions
                p_vec[self.idx_p_qdi:self.idx_p_qdi + nq] = np.asarray(param.qdi, dtype=float).reshape(nq,)
                p_vec[self.idx_p_foi:self.idx_p_foi + 3*n_foi] = np.asarray(param.FOI_position, dtype=float).reshape(3*n_foi,)
                # BOS half-spaces (kept last)
                p_vec[-6:] = p_bos_vals
                ocp_solver.set(k, "p", p_vec)
        else:
            # Non-free-flyer: set qdi parameter at all stages
            p_vec = np.copy(ocp.parameter_values)
            p_vec[self.idx_p_qdi:self.idx_p_qdi + nq] = np.asarray(param.qdi, dtype=float).reshape(nq,)
            p_vec[self.idx_p_foi:self.idx_p_foi + 3*n_foi] = np.asarray(param.FOI_position, dtype=float).reshape(3*n_foi,)
            for k in range(param.nb_samples + 1):
                ocp_solver.set(k, "p", p_vec)
...
#set the cost function and target weigths. For target default uses ramp interpolation 0-->1 at desired time index
    ...
    def _compute_initial_contact_wrenches(self, q0):
        """Compute quasi-static initial contact wrenches (in local foot frames).
        Returns (F_lf, F_rf) as pin.Force.
        """
        pin.forwardKinematics(self.model, self.data, q0)
        pin.updateFramePlacements(self.model, self.data)

        tau_ff0 = pin.rnea(self.model, self.data, q0, np.zeros(self.nv), np.zeros(self.nv))
        tau_ff0 = np.asarray(tau_ff0)

        force_ff = pin.Force(tau_ff0[:3].reshape(3,), tau_ff0[3:6].reshape(3,))
        F_world = self.data.oMi[0].act(force_ff)

        # Split load evenly as initial guess
        F_lf = self.M_lf.inverse().actInv(F_world / 2.0)
        F_rf = self.M_rf.inverse().actInv(F_world / 2.0)
        return F_lf, F_rf
    def calculate_cost(self, cx, cu, param, p):
        """
        ...
        for i, foi_id in enumerate(param.FOI_to_set_Id):
            block6 = self.cost_to_target_expr(cx, cu[:nv], p)[i*6:(i+1)*6]
    ...
    def approx_log6_RT(self, frame_id, R_target, t_target, cx, cu):
        """Like approx_log6, but with target (R,t) where t can be symbolic (from parameters)."""
        cpin.forwardKinematics(self.cmodel, self.cdata, cx[:self.nq], cx[self.nq:], cu)
        cpin.updateFramePlacements(self.cmodel, self.cdata)

        tran_error = self.cdata.oMf[frame_id].translation - t_target
        R_err = self.cdata.oMf[frame_id].rotation.T @ R_target
        rot_error = self.symbolic_log3(R_err)
        return casadi.vertcat(tran_error, rot_error)

    def cost_to_target_expr(self, cx, cu, p):
        """Stacked 6D log errors for all FOIs, using FOI positions from the parameter vector p."""
        errs = []
        for i, foi_id in enumerate(self.FOI_ids):
            t_i = p[self.idx_p_foi + 3*i : self.idx_p_foi + 3*(i+1)]
            errs.append(self.approx_log6_RT(foi_id, self.FOI_R[i], t_i, cx, cu))
        return casadi.vertcat(*errs) if len(errs) else casadi.SX.zeros(0, 1)
