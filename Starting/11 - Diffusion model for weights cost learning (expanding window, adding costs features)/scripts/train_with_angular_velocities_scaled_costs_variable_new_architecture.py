...
import joblib  # Pour sauvegarder les scalers proprement
...
DATASET_PATH = "data/dataset_parallel_299862_samples.pkl"
...
class VariableLengthDataset(Dataset):
    def __init__(self, data_path, scaler_traj=None, scaler_w=None, random_slice=True, min_len=20):
        ...
        print("Normalizing data in memory...")
        ...
        # Since lengths vary, we must iterate. 
        # But scaling one by one is slow. We can concat, scale, split, or just loop if fast enough.
        # Loop is safer for memory with ragged arrays.
        self.traj_data = []
...
def collate_fn(batch):
    ...
    # Our updated Transformer Encoder expects (B, 4, L) input which it internally permutes.
...
def prepare_scalers(dataset_path):
    ...
    # We take a subset of points to fit scaler to save RAM, or all if possible.
    # Let's stack 10000 random steps from trajectories to estimate statistics
...
def main():
    ...
    if os.path.exists(scaler_w_path) and os.path.exists(scaler_traj_path):
        ...
        scaler_traj, scaler_w = prepare_scalers(target_file)
